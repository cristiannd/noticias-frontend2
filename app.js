...
for(let i=0; i<noticias.length; i++){
  const mainNoticias = document.querySelector('main')
  mainNoticias.innerHTML += `
    <article class="tarjeta">
    <div class="tarjeta-izquierda">
      <div class="tarjeta-izquierda-header">
        <p class="ubicacion-articulo">${noticias[i].tipoNacional ? "Nacional" : "Internacional"}</p>
        <p class="fecha-articulo">${noticias[i].fecha}</p>
      </div>
      <h3>${noticias[i].titulo}</h3>
      <p class="descripcion-articulo">${noticias[i].descripcion}</p>
    </div>
    <div class="tarjeta-derecha">
      <img src="${noticias[i].imgUrl}" alt="imagen de ${noticias[i].titulo}">
    </div>
    </article>
  `
}